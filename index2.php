...
curl_setopt_array($ch, array(
    CURLOPT_URL => 'http://check.getipintel.net/check.php?ip='. $_SERVER['REMOTE_ADDR'] .'&contact=meltstrap@uiiverse.xyz&flags=f',
    CURLOPT_RETURNTRANSFER => true));
$is_proxy = curl_exec($ch);
curl_close($ch);

if ($is_proxy > 0.95) {
    exit('You can\'t access Uiiverse when using a proxy.');
}
...
array('GET|POST', '/android/', 'android/index.php', 'Android'),
array('GET|POST', '/wii/', 'wii/index.php', 'Wii'),
array('GET|POST', '/desktop/', 'desktop/index.php', 'Desktop')
));
